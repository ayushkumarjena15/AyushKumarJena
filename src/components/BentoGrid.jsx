        ...
        <div className="relative w-full h-full flex items-center justify-center bg-background min-h-[220px] select-none">
            ...
            <div className="relative w-44 h-44 md:w-52 md:h-52 rounded-full p-1 bg-gradient-to-br from-[#222] via-[#111] to-[#333] shadow-[0_20px_50px_rgba(0,0,0,0.8),inset_0_-2px_5px_rgba(255,255,255,0.1)]">
                ...
                <div className="relative w-full h-full rounded-full bg-[#0c0c0c] overflow-hidden shadow-[inset_0_2px_10px_rgba(0,0,0,1)] border border-white/5">
                    ...
                    {/* Jacob & Co Celestial Background */}
                    <div className="absolute inset-0 bg-[#050505]" />

                    {/* Rotating Skeletonized Gears (Functional Background) */}
                    <motion.div
                        className="absolute inset-0 opacity-20 border-[20px] border-double border-white/5 rounded-full"
                        animate={{ rotate: 360 }}
                        transition={{ duration: 60, repeat: Infinity, ease: "linear" }}
                    >
                        <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-40 h-40 rounded-full border border-white/10 border-dashed" />
                    </motion.div>

                    {/* Spinning Mechanical Gears */}
                    <motion.div
                        className="absolute top-[25%] left-[25%] w-12 h-12 bg-white/5 rounded-full border border-white/10 flex items-center justify-center"
                        animate={{ rotate: -360 }}
                    ...
                    <motion.div
                        className="absolute bottom-[20%] right-[30%] w-16 h-16 bg-white/5 rounded-full border border-white/10 flex items-center justify-center"
                        animate={{ rotate: 360 }}
                        transition={{ duration: 15, repeat: Infinity, ease: "linear" }}
                    >
                        {[...Array(8)].map((_, i) => (
                            <div key={i} className="absolute w-[2px] h-3 bg-white/20" style={{ transform: `rotate(${i * 45}deg) translateY(-28px)` }} />
                        ))}
                    </motion.div>

                    {/* Minute Markers (Floating Style) */}
                    {[...Array(12)].map((_, i) => (
                        <div
                            key={i}
                        ...
                        >
                            <div className="absolute top-3 left-1/2 -translate-x-1/2 text-[8px] font-black text-white/40 font-heading">
                                {i === 0 ? 12 : i}
                            </div>
                        </div>
                    ))}

                    {/* Date Window (Skeletonized) */}
                    <div className="absolute right-[22%] top-1/2 -translate-y-1/2 w-8 h-8 rounded-full bg-[#111] border border-accent1/30 flex items-center justify-center shadow-lg z-10">
                        <span className="text-[10px] font-bold text-accent1">{date}</span>
                    </div>

                    {/* Floating Branding Sub-dial */}
                    <div className="absolute top-[28%] left-1/2 -translate-x-1/2 z-10">
                        <div className="px-2 py-0.5 rounded-full bg-accent1/10 border border-accent1/20">
                            <p className="text-[6px] font-black tracking-[0.2em] text-accent1 uppercase">ASTRONOMIA</p>
                        </div>
                    </div>

                    {/* Hour Hand (Skeleton Blue-Steel Style) */}
                    <motion.div
                        className="absolute bottom-1/2 left-1/2 w-[5px] h-[24%] origin-bottom z-20"
                        animate={{ rotate: hourDeg }}
                        transition={{ type: "spring", stiffness: 50, damping: 20 }}
                        style={{ x: "-50%" }}
                    >
                        <div className="w-full h-full bg-gradient-to-t from-blue-600 to-blue-400 rounded-sm shadow-xl relative">
                            <div className="absolute inset-[1px] bg-[#0c0c0c] rounded-sm" />
                        </div>
                    </motion.div>

                    {/* Minute Hand (Skeleton Blue-Steel Style) */}
                    <motion.div
                        className="absolute bottom-1/2 left-1/2 w-[3px] h-[36%] origin-bottom z-30"
                        animate={{ rotate: minuteDeg }}
                        transition={{ type: "spring", stiffness: 50, damping: 20 }}
                        style={{ x: "-50%" }}
                    >
                        <div className="w-full h-full bg-gradient-to-t from-blue-600 to-blue-400 rounded-full shadow-xl relative">
                            <div className="absolute inset-[1px] bg-[#0c0c0c] rounded-full" />
                        </div>
                    </motion.div>

                    {/* Functional Central Tourbillon (Seconds) */}
                    <div
                        className="absolute bottom-1/2 left-1/2 w-[2px] h-[45%] origin-bottom z-40"
                        style={{ transform: `translateX(-50%) rotate(${secondDeg}deg)` }}
                    >
                        {/* Spinning Diamond/Planet element on the second hand */}
                        <motion.div
                            className="absolute top-0 left-1/2 -translate-x-1/2 w-4 h-4"
                            animate={{ rotate: 360 }}
                            transition={{ duration: 2, repeat: Infinity, ease: "linear" }}
                        >
                            <div className="w-full h-full bg-accent1 rounded-sm rotate-45 shadow-[0_0_15px_rgba(194,160,122,0.8)] border border-white/20" />
                        </motion.div>
                        <div className="w-full h-full bg-accent1/80 rounded-full" />
                    </div>

                    {/* Center Core Cap (Diamond Style) */}
                    <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-6 h-6 z-50">
                        <div className="absolute inset-0 bg-accent1/20 rounded-full blur-md" />
                        <div className="relative w-full h-full bg-gradient-to-br from-[#444] to-[#111] rounded-full border border-white/30 flex items-center justify-center shadow-2xl">
                            <div className="w-2 h-2 bg-white rounded-full shadow-[0_0_10px_#fff]" />
                        </div>
                    </div>

                    {/* Domed Glass Effect Overlay */}
                    <div className="absolute inset-0 pointer-events-none z-[60] bg-[radial-gradient(circle_at_30%_30%,_rgba(255,255,255,0.1)_0%,_transparent_60%)] opacity-50" />
                    <div className="absolute inset-0 border-[15px] border-black/40 rounded-full pointer-events-none z-[60]" />
                </div>
            </div>
        </div>
    );
